...
public class ListImageSize {
    private final boolean IS_LAST_FROM_GROUP;

    public ListImageSize(boolean lastFromGroup) {
    ...
    public static class Adapter extends RecyclerView.Adapter<Adapter.MyViewHolder> {
        private final ListImageSize LIST_IMAGE_SIZE;

        public Adapter(ListImageSize listCameraSize) {
            this.LIST_IMAGE_SIZE = listCameraSize;
        }
        ...
        public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_list_image_size, parent, false));
        }
        ...
        public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            ...
            if(ks.getImagePath(false) == null || !new File(ks.getImagePath(false)).exists()) {
                holder.EL_IMAGE_EXAMPLE.setImageResource(R.drawable.icon_kapture_image_helper);
            } else {
                holder.EL_IMAGE_EXAMPLE.setImageBitmap(BitmapFactory.decodeFile(ks.getImagePath(false)));
            }

            if(!LIST_IMAGE_SIZE.isLastItemFromGroup()) {
            ...
            holder.EL_SLIDER.addOnChangeListener((slider, value, fromUser) -> {
                ...
                sp.edit().putInt(Constants.Sp.Profile.IMAGE_SIZE, (int) value).apply();
            });

            holder.EL_SLIDER.setValue(sp.getInt(Constants.Sp.Profile.IMAGE_SIZE, DefaultSettings.IMAGE_SIZE));
        }
